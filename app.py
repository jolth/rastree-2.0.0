...
if web.config.get('_sesion') is None:
    ...
    #session = web.session.Session(app, web.session.DiskStore(os.path.join(curdir,'sesiones')), {'count': 0})
    #web.config._sesion = session
else:
    session = web.config._sesion

def session_hook():
    web.ctx.session = session # Session available in Sub-apps
    web.template.Template.globals['sesion'] = session # Sessions avaible in templates
...
class Index:
    def GET(self):
        if session.get('logged_in', False):
            ...
            if referer is web.ctx.homedomain: raise  web.seeother('/login') # Resuelve el Referer 
